...
async function lupaPinReset() {
  ...
  if (inputBypass === MASTER_RESET_CODE) {
    isPinActive = false;
    pinCode = '1234';
    await db.from('pengaturan').update({ pin_code: '1234', is_pin_active: false }).eq('id', 'config');
...
async function toggleFiturPin() {
  ...
  await db.from('pengaturan').update({ is_pin_active: isPinActive }).eq('id', 'config');
...
async function simpanPinBaru() {
  ...
  pinCode = baru;
  await db.from('pengaturan').update({ pin_code: baru }).eq('id', 'config');
  tutupModalPin();
  showToast('PIN server berhasil diganti!', 'success');
}
