...
// Backup Cepat: Baca dari memori HP dulu biar tidak nunggu loading jaringan
let pinCode = localStorage.getItem('vp_pin_' + MY_DEVICE_ROLE) || '1234';
let isPinActive = localStorage.getItem('vp_pin_active_' + MY_DEVICE_ROLE) === 'true';
...
window.addEventListener('load', async function() {
  ...
  // Terapkan PIN instan dari LocalStorage sebelum Supabase loading
  applyPinUI();
...
async function loadKeamananServer() {
  try {
    const { data } = await db.from('pengaturan').select('*').eq('id', 'config').single();
 db.from('pengaturan').select('*').eq('id', 'config').single();
    if (data) {
      if (MY_DEVICE_ROLE === 'user_1') {
        pinCode = String(data.pin_u1 || '1234');
        isPinActive = (data.is_pin_active_u1 === true || data.is_pin_active_u1 === 'true');
      } else {
        pinCode = String(data.pin_u2 || '1234');
        isPinActive = (data.is_pin_active_u2 === true || data.is_pin_active_u2 === 'true');
      }
      localStorage.setItem('vp_pin_' + MY_DEVICE_ROLE, pinCode);
      localStorage.setItem('vp_pin_active_' + MY_DEVICE_ROLE, isPinActive);
    }
  } catch(e) {}
...
function bukaModalPreferences() { triggerVibration(20); document.getElementById('modal-preferences')?.classList.add('active'); }
function tutupModalPreferences() { document.getElementById('modal-preferences')?.classList.remove('active'); }
...
async function lupaPinReset() {
  ...
  if (inputBypass === MASTER_RESET_CODE) {
    isPinActive = false;
    pinCode = '1234';
    localStorage.setItem('vp_pin_' + MY_DEVICE_ROLE, '1234');
    localStorage.setItem('vp_pin_active_' + MY_DEVICE_ROLE, 'false');
    
    let updatePayload = MY_DEVICE_ROLE === 'user_1' ? { id: 'config', pin_u1: '1234', is_pin_active_u1: false } : { id: 'config', pin_u2: '1234', is_pin_active_u2: false };
    await db.from('pengaturan').upsert(updatePayload);
...
function bukaModalSecurity() { triggerVibration(20); document.getElementById('modal-security-settings')?.classList.add('active'); }
function tutupModalSecurity() { document.getElementById('modal-security-settings')?.classList.remove('active'); }
...
async function toggleFiturPin() {
  ...
  let updatePayload = MY_DEVICE_ROLE === 'user_1' ? { id: 'config', is_pin_active_u1: isPinActive } : { id: 'config', is_pin_active_u2: isPinActive };

  // Menggunakan UPSERT agar jika data belum ada, otomatis dibuatkan di server
  const { error } = await db.from('pengaturan').upsert(updatePayload);
  if (error) {
    showToast('Gagal simpan ke database: ' + error.message, 'error');
  } else {
    showToast(isPinActive ? 'PIN diaktifkan' : 'PIN dinonaktifkan', 'success');
  }
}

function bukaModalPin() { triggerVibration(20); document.getElementById('modal-edit-pin')?.classList.add('active'); }
function tutupModalPin() {
  document.getElementById('modal-edit-pin')?.classList.remove('active');
...
async function simpanPinBaru() {
  ...
  let updatePayload = MY_DEVICE_ROLE === 'user_1' ? { id: 'config', pin_u1: baru } : { id: 'config', pin_u2: baru };

  const { error } = await db.from('pengaturan').upsert(updatePayload);
  tutupModalPin();
  if (error) {
    showToast('PIN gagal tersimpan di server!', 'error');
  } else {
...
function tutupModalNama() { document.getElementById('modal-edit-nama')?.classList.remove('active'); }
